...
import { maskKey, buildKeyHeaders } from "@/lib/api-keys";
import {
  X,
  Settings,
  Key,
...
const KEY_CONFIG = [
  {
    id: "geminiKey" as const,
    label: "Gemini API Key",
    envName: "GEMINI_API_KEY",
    placeholder: "AIza...",
    description: "Powers AI-driven analysis. Get one at ai.google.dev",
  },
  {
    id: "productboardKey" as const,
    label: "Productboard API Token",
    envName: "PRODUCTBOARD_API_TOKEN",
    placeholder: "pb_...",
    description: "Fetches features and notes from Productboard",
  },
  {
    id: "attentionKey" as const,
    label: "Attention API Key",
    envName: "ATTENTION_API_KEY",
    placeholder: "att_...",
    description: "Fetches call recordings from Attention",
  },
];
...
export function SettingsDialog({ open, onClose }: SettingsDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      for (const cfg of KEY_CONFIG) {
        initial[cfg.id] = {
          value: keys[cfg.id],
          visible: false,
          dirty: false,
          validating: false,
          valid: keys[cfg.id] ? true : null,
          error: null,
        };
      }
      setFields(initial);
  ...
  const updateField = useCallback(
    (id: string, updates: Partial<KeyFieldState>) => {
      setFields((prev) => ({
        ...prev,
        [id]: { ...prev[id], ...updates },
      }));
  ...
  async function handleValidate(id: string) {
    ...
    try {
      const testKeys = { ...keys, [id]: field.value };
      const res = await fetch("/api/settings/validate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...buildKeyHeaders(testKeys),
        },
        body: JSON.stringify({ keyName: id }),
      });
      ...
      updateField(id, {
        validating: false,
        valid: false,
        error: "Could not reach validation endpoint",
      });
    }
  }
  ...
  function handleSave(id: string) {
    ...
    if (trimmed) {
      setKey(id as keyof typeof keys, trimmed);
    } else {
      removeKey(id as keyof typeof keys);
    }
    updateField(id, { dirty: false });
    setSaveMessage(`${KEY_CONFIG.find((c) => c.id === id)?.label} saved`);
  ...
  function handleRemove(id: string) {
    ...
    setSaveMessage(`${KEY_CONFIG.find((c) => c.id === id)?.label} removed`);
    setTimeout(() => setSaveMessage(null), 2000);
    setTimeout(() => refreshStatus(), 500);
  }
      ...
      <div className="relative bg-background border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">
                Configure keys to connect live data sources
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">
                  Display sample data when no API keys are configured
                </p>
              </div>
              <button
                ...
                className={cn(
                  ...
                  useDemoData ? "bg-primary" : "bg-muted-foreground/30"
                )}
              >
                <div
                  className={cn(
                    "absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform",
                    useDemoData ? "translate-x-5" : "translate-x-0.5"
                  )}
                />
              </button>
            </div>
          ...
          {KEY_CONFIG.map((cfg) => {
            ...
            const envConfigured = status[cfg.id]?.source === "env";
              ...
              <div key={cfg.id} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {keys[cfg.id] && !field.dirty && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-600 font-medium">
                        Saved
                      </span>
                    )}
                  </div>
                ...
                <div className="flex gap-2">
                  <div className="flex-1 relative">
                    <input
                      ...
                      value={field.value}
                      onChange={(e) =>
                        updateField(cfg.id, { value: e.target.value, dirty: true })
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        updateField(cfg.id, { visible: !field.visible })
                      }
                  ...
                  <button
                    ...
                    className={cn(
                      "px-3 py-2 rounded-lg text-[10px] font-medium flex items-center gap-1.5 transition-colors",
                      field.dirty || !keys[cfg.id]
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={!field.value.trim() || field.validating}
          ...
          <div className="p-3 rounded-xl bg-muted/30 border border-border space-y-2">
            ...
            <ul className="text-[10px] text-muted-foreground space-y-1">
              ...
              <li>They are sent securely to the server on each request</li>
              <li>You can add just one key at a time — not all are required</li>
              <li>Environment variables (.env) are also supported as fallback</li>
              <li>Use &ldquo;Test Connection&rdquo; to verify a key works</li>
            </ul>
          </div>
        </div>
      </div>
